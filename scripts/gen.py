...
class CodeGenerator:
    ...
    def __init__(self, platform: str, prefix: str, lib_name: str, lib_dir: str, name_space: str):
    ...
    def parse_source_file(self, file_path: pathlib.Path, include_paths: Optional[List[str]] = None) -> None:
        ...
        print(f"Parsing file: {file_path}, args: {args}")
...
namespace {self.name_space}::{self.platform}
{{
{self.typedefs_str}
class Driver
{{
private:
    /// FIXME
    DEFINE_GET_SYMBOL_FUNC(GetSymbol, "{self.platform.upper()}_LIB",
...
using namespace {self.name_space}::{self.platform};
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name-space', type=str, required=False, default="cuda_intercept", help='Name space for the generated classes')
    ...
    generator = CodeGenerator(args.platform, args.prefix, lib_path.name, str(lib_path.parent), args.name_space)
